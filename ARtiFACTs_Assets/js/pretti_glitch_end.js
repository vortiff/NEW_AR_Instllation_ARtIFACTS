    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        dracoLoader.setDecoderPath('https://cdn.jsdelivr.net/gh/mrdoob/three.js@master/examples/jsm/libs/draco/gltf/');
        const ktx2Loader = new KTX2Loader();
        ktx2Loader.setTranscoderPath('https://cdn.jsdelivr.net/gh/mrdoob/three.js@master/examples/js/libs/basis/');
        ktx2Loader.detectSupport(renderer);
        const loader = new GLTFLoader();
        loader.setDRACOLoader(dracoLoader);
        loader.setKTX2Loader(ktx2Loader);
        loader.load('https://cdn.jsdelivr.net/gh/vortiff/NEW_AR_Instllation_ARtIFACTS@main/ARtiFACTs_Assets/glb/NewMapAnimationTest.glb', function(gltf) {
            model = gltf.scene;;    
            ...
            model.traverse((child) => {
                if (child.isMesh) {
                    console.log(`Name: ${child.name}, Material: ${child.material.name}, Map: ${child.material.map}`);
                }
            });
            animate();
            ...
            model.traverse((child) => {
                if (child.isMesh && meshNames.includes(child.name)) {
                    ...
                    if (!originalVertices[child.name]) {
                        originalVertices[child.name] = child.geometry.attributes.position.array.slice();
                    }
                    ...
                    for (let i = 0; i < vertices.length; i += 3) {
                        ...
                        vertices[i] = original[i] + simplex.noise3D(original[i] * scale, original[i + 1] * scale, time * speed) * glitchAmount;     // x
                        vertices[i + 1] = original[i + 1] + simplex.noise3D(original[i + 1] * scale, original[i + 2] * scale, time * speed) * glitchAmount; // y
                        vertices[i + 2] = original[i + 2] + simplex.noise3D(original[i] * scale, original[i + 2] * scale, time * speed) * glitchAmount; // z
                    }        
        ...
        function animate() {
            ...
            animateMeshes(["Icosphere", "Icosphere001", "Icosphere002", "Icosphere003", "Icosphere004", "Icosphere005", "Icosphere006"]);
        ...
        let wrapDocElement = document.querySelector('[data-w-id="aaf856a2-deeb-f0de-0eec-bcaced7c4287"]');
        if (container) {
        ...
        function onDocumentMouseDown(event) {
            ...
            if (intersects.length > 0) {
                ...
                if (intersects.length > 0) { /// Se c'è almeno un oggetto intersecato
                    ...
                    if (dataWId) {
                        ...
                        const wrapDocElement = document.querySelector(`[data-w-id='${dataWId}']`);
                        if(wrapDocElement) {
